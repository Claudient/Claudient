 ...
 * Demonstrates complete AWS deployment workflow for UitKit
 */

const AWSDeployment = require('./aws-deployment');
const path = require('path');

async function deployUitKitToAWS() {
...
exports.handler = async (event) => {
  console.log('UitKit dont-stop task triggered');
  console.log('Event:', JSON.stringify(event, null, 2));

  return {
    statusCode: 200,
    body: JSON.stringify({
      message: 'UitKit dont-stop task executed successfully',
...
`;
    ...
    const lambdaResult = await deployment.deployLambda({
      functionName: 'uitkit-dont-stop-task',
      runtime: 'nodejs18.x',
      ...
      environment: {
        SKILLS_BUCKET: 'uitkit-skills-storage',
        STATE_TABLE: 'uitkit-deployment-state',
        ENVIRONMENT: 'production',
      },
      description: 'UitKit dont-stop task executor - maintains deployment continuity',
    });
    ...
    const eventRuleResult = await deployment.createLambdaEventRule({
      functionName: 'uitkit-dont-stop-task',
      ruleName: 'uitkit-dont-stop-schedule',
      scheduleExpression: 'rate(5 minutes)',
      description: 'Trigger UitKit dont-stop task every 5 minutes',
    });
    ...
    const ecsResult = await deployment.deployECS({
      clusterName: 'uitkit-ecs-cluster',
      serviceName: 'uitkit-orchestrator',
      taskDefinitionFamily: 'uitkit-orchestrator-task',
      desiredCount: 2,
      launchType: 'FARGATE',
      image: 'your-registry/uitkit-orchestrator:latest',
      cpu: '256',
      memory: '512',
      containerPort: 8080,
      environment: {
      ...
      logGroupName: '/ecs/uitkit/orchestrator',
    ...
    const templateConfig = {
      stackName: 'uitkit-main-stack',
      resources: {
        ecsCluster: {
          name: 'uitkit-ecs-cluster',
    ...
    const ecsHealthCheck = await deployment.healthCheckECS('uitkit-ecs-cluster', 'uitkit-orchestrator');
    console.log(`ECS Health Check:`, ecsHealthCheck);

    const lambdaHealthCheck = await deployment.healthCheckLambda('uitkit-dont-stop-task');
    ...
    console.log('\n✓ UitKit AWS deployment completed successfully');
...
module.exports = { deployUitKitToAWS, redeployFromState, estimateCosts };
