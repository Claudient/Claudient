...
class AWSDeployment extends EventEmitter {
  constructor(config = {}) {
    ...
    this.tags = config.tags || { ManagedBy: 'UitKit', Environment: 'production' };
  }
  ...
  async deployLambda(config) {
    ...
    try {
      let createCmd = this.buildAwsCommand([
        ...
        `--description=${description || `UitKit dont-stop task: ${functionName}`}`,
      ]);
  ...
  generateCloudFormationTemplate(config) {
    ...
    const template = {
      ...
      Description: `UitKit deployment stack: ${stackName}`,
      Resources: {},
      Outputs: {},
    };
