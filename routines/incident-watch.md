...
Fires when an alert or webhook signals a potential incident — a spike in errors, a failed health check, a PagerDuty alert, or any external signal routed to the routine's API endpoint. Within seconds of receiving the signal, the agent gathers structured context: recent error logs, the last few deployments, open PRs merged in the past 24 hours, and any related GitHub issues. It then drafts a concise incident summary and posts it to a configured Slack channel or incident management tool. The goal is to cut the first 5–10 minutes of frantic context-gathering during an incident.
...
- Payload schema (minimum required):
  ...
  ```
- Can also be triggered manually via API for drills or retrospective analysis

## Setup

1. Register the routine's API endpoint in your alerting platform as a webhook destination.
...
3. Configure log source access — at least one of:
...
- On log fetch failure: the summary is still posted with a note that log data is unavailable — the routine never blocks on a failed data source
- On duplicate alert (same title within 10 minutes): the routine detects this and replies in-thread on the existing Slack message rather than posting a new one
...
**Slack message posted to #incidents:**
...
**GitHub issue opened:** #402 "Incident: payments-service error spike — 2026-06-10"
