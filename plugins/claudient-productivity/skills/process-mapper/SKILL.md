...
## When to activate
- Documenting an existing process that lives only in someone's head
- Mapping a new process before you build an SOP or training material
- Identifying where handoffs break, steps slow down, or work gets dropped
- Building a RACI matrix for cross-functional processes with unclear ownership
- Preparing for a process audit, ISO certification, or operational review
- Before automating a workflow — map it manually first, then eliminate waste

## When NOT to use
- You already have a fully documented and current process — use `/sop-writer` to update it
- The process involves only one person with no handoffs — use a simple checklist
- You're mapping a technical system architecture — that's a different diagram type
- The process doesn't exist yet and you need to design it from scratch — start with a workflow design session first

## Instructions

### Full process mapping prompt
...
### Rapid process sketch (10-minute version)
...
**User:** Map our vendor procurement process. It starts when a team lead requests a new tool and ends when the vendor is contracted and access is provisioned. Involves: Team Lead, Ops Manager, Finance, Legal, IT.
