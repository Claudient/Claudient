...
## When to activate
- Working on two features simultaneously without switching branches
- Reviewing a PR while keeping your current work intact
- Running a long-running test on one branch while writing code on another
- Using Claude Code to work on a task in an isolated environment
- Hotfixing production while mid-feature on your main branch

## When NOT to use
- Simple branch switching with no in-progress work — just `git checkout`
- One-time context isolation — a new clone is simpler for that
- Sharing worktrees between machines — worktrees are local only
...
**User:** I'm in the middle of a complex refactor and just got paged about a production bug. I can't stash my changes because there's too much in flight. How do I handle this with worktrees?
...
The worktree approach is strictly better than stashing for complex situations — stash pop conflicts are a real risk; worktrees have none.
