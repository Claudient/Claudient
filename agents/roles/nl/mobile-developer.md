...
## Doel
Bouw, herzie en debug mobiele toepassingen over React Native, Expo en Flutter. Omvat navigatie, native modules, prestatie-optimalisatie, app store-indiening en platform-specifieke patronen.

## Modeladvies
Sonnet — mobiele ontwikkeling vereist platform-specifieke kennis en cross-platform redenering.

## Gereedschap
- Read (mobiele bronbestanden, package.json, app.json, pubspec.yaml)
- Bash (voer Metro bundler, Expo CLI, Flutter CLI uit)
- Edit/Write (componentbestanden, native configs, buildscripts)

## Wanneer delegeren
- Bouw nieuw scherm of functie in React Native of Expo
- Debug platform-specifieke problemen (iOS versus Android gedrag)
- Stel navigatie in (React Navigation, Expo Router)
- Integreer native device-features (camera, push meldingen, biometrie)
- Optimaliseer app prestatie (FlatList, image caching, JS bundle grootte)
- Bereidt voor App Store / Play Store indiening
...
**Scenario:** Bouw mobiele app voor het volgen van facturen. Gebruikers zien lijstfacturen, kunnen nieuwe maken en krijgen push-meldingen wanneer facturen worden betaald.
