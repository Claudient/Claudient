# Operations Manager Workspace — Project Structure

> For an operations manager running business processes, vendor management, team ops, and cross-functional coordination — with Notion, Linear, Slack, Google Workspace, Zapier, Airtable, and Monday.com as the operating stack.

## Stack

- **Notion** or **Confluence** — SOP library, process documentation, internal wiki, runbooks
- **Linear** or **Asana** — Project tracking, cross-functional initiatives, sprint-style task management
- **Slack** — Team comms, ops channel, incident threads, vendor escalation channels
- **Google Workspace** — Drive, Sheets (metrics tracking), Docs (reports), Calendar (meeting facilitation)
- **Zapier** or **Make** — Workflow automation, form-to-tracker pipelines, Slack notification rules
- **Airtable** — Vendor registry, contract tracking, OKR dashboard, approval workflows
- **Monday.com** — Cross-team project visibility, capacity planning, roadmap views
- **Claude Code** — SOP drafting, process mapping, vendor reviews, OKR updates, weekly ops reports, meeting action distillation
...
| Path | Purpose |
|---|---|
| `.claude/commands/sop-draft.md` | Slash command that takes a process name and context, outputs a complete SOP with purpose, scope, RACI table, step-by-step procedure, exception handling, and review cadence |
| `.claude/commands/vendor-review.md` | Slash command that takes a vendor name and review period, outputs a structured performance review against SLA terms with renewal recommendation |
| `.claude/commands/okr-update.md` | Slash command that takes current OKR state and progress notes, outputs a formatted OKR status update with confidence scores and risk flags per key result |
| `.claude/commands/weekly-ops.md` | Slash command to generate the weekly ops report — aggregates wins, blockers, cross-team dependencies, and open action items into a shareable doc |
| `.claude/commands/meeting-actions.md` | Slash command that takes raw meeting notes and outputs a clean action item table with owners, descriptions, and due dates |
| `sops/_template/sop-template.md` | Canonical SOP structure every process document follows — ensures consistency across departments and makes SOPs parseable by Claude commands |
| `vendors/vendor-registry.csv` | Single source of truth for all active vendors — name, category, contract end date, spend tier, DRI — drives the quarterly review schedule |
| `okrs/2026/q2/ops-team-okrs.md` | Current quarter ops team OKRs — the live document updated weekly with confidence scores and updated as the quarter progresses |
| `automation/_automation-index.md` | Master index of every active Zapier and Make automation — prevents duplication, documents owners, and flags automations needing re-testing |
| `reports/weekly/_report-template.md` | Standard weekly ops report template — ensures every report covers the same sections so stakeholders know where to find what they need |
